...
body {
	background-image:
		url("https://img.etimg.com/thumb/height-480,width-640,msid-62473601,imgsize-15688/balance-sheet-thinkstock.jpg");
...
<div class="header">
	...
	<div class="header-right">
		<a href="/user-home"><i class="fas fa-home"></i> HOME</a>
		 <a href="/user-reports"><span class="glyphicon glyphicon-stats"></span> REPORTS</a> <a
			href="/user-transaction/all-transactions"><i class="fas fa-history"></i> TRANSACTIONS</a>
	</div>
</div>
...
<br>
<br>
	<h1 style="color: blue">Updating Transaction</h1>
...
<form:input path="id" type="hidden" value="${transaction.getId()}"/>
			...
			<div>
				...
				<form:input type="date" path="date" name="date"
					value="${transaction.getDate()}" class="keyup-date text-input" id="datepicker"/>
			</div>

			<div>
				<span>Amount:</span>
				<form:input type="number" path="amount" name="amount"  min="1" max="1000000"
					value="${transaction.getAmount()}" id=""/>
			</div>
			<br>
			<br>
			<center>
 ...
 */
    ...
    if(!dateReg.test(inputVal)) {
        $(this).after('<span class="error error-keyup-5" style="color: brown">Invalid date format.</span>');
    }
});
</script>
	</div>
