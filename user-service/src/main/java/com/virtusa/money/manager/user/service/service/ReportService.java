...
public class ReportService {
	...
	public Report getForReport(Long id, Long year) {
		...
		allTransaction.stream().filter(i -> i.getCategoryList().getCategory().getCategory().equals("income"))
				...
				.forEach(i -> incomeForYear.set(LocalDate.parse(i.getDate()).getMonthValue(), incomeForYear.get(LocalDate.parse(i.getDate()).getMonthValue())+i.getAmount()));
		
		allTransaction.stream().filter(i -> i.getCategoryList().getCategory().getCategory().equals("expense"))
				.sorted(Comparator.comparing(UserTransaction::getDate)).filter(basedOnYear(year))
				.forEach(i -> expenseForYear.set(LocalDate.parse(i.getDate()).getMonthValue(), expenseForYear.get(LocalDate.parse(i.getDate()).getMonthValue())+i.getAmount()));
		
		allTransaction.stream().filter(i -> i.getCategoryList().getCategory().getCategory().equals("inverstment"))
				.sorted(Comparator.comparing(UserTransaction::getDate)).filter(basedOnYear(year))
				.forEach(i -> investmentForYear.set(LocalDate.parse(i.getDate()).getMonthValue(), investmentForYear.get(LocalDate.parse(i.getDate()).getMonthValue())+i.getAmount()));
